...
const {function_name} =sql`\n{sql_query}`;\n\n"""
            ...
            # result = subprocess.run(['npx', 'pgtyped', '-c', cfg, '-f', file_override], capture_output=True, text=True)

            # # Split the stderr into lines
            # error_lines = result.stderr.splitlines()

            # # The last line of the stderr should be the output of the last console.error() call
            # last_error_line = error_lines[-1] if error_lines else None
