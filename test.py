...
def main():
    ...
    # select_result = sql("SELECT * FROM us_federal_ecfr WHERE node_type = 'content' AND status is NULL LIMIT 5;", "SelectFederalRows")
    select_result: List[SelectFederalRowsResult] = sql("SELECT * FROM us_federal_ecfr WHERE node_type = :content_type AND status is NULL LIMIT :limit;", SelectFederalRows)
