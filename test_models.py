...
class SelectFederalRows:
	...
	Original SQL: "SELECT * FROM us_federal_ecfr WHERE node_type = 'content' AND status is NULL LIMIT 7"
	Used in files: []
	"""
...
class SelectBadRows:
	...
	Class to hold all pydantic models for a single SQL query.
	Defined by SQL invocation in test.py.
	Original SQL: "SELECT * FROM us_federal_ecfr WHERE node_type = 'content' AND status is not NULL LIMIT 10"
	Used in files: []
	"""
